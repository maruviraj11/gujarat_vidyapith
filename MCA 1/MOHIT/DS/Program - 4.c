//Program - 4 : Write menu driven program to Implement simple queue 1) insert 2) delete 3) modify 4) display
#include<stdio.h>
#include<stdlib.h>
# define max 5
...
void enqueue(int *f,int *r,int q[max],int nval)
{
	if(*r==max-1)
	{
...
int dequeue(int *f,int *r,int q[max])
{
	int temp;
	if(*f==-1 && *r==-1)
	{
